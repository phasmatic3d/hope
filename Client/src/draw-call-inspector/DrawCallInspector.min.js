...
const THREE_GLOBAL = Object.assign({}, _THREE);
globalThis.THREE = THREE_GLOBAL;
const THREE = THREE_GLOBAL;
...
(function () {
    ...
    var u = new THREE.OrthographicCamera(-1, 1, 1, -1, 0, 1),
        r = new THREE.Mesh(new THREE.PlaneGeometry(2, 2), new THREE.MeshBasicMaterial());
    k.add(r);
    ...
    d.prototype = {
        ...
        mount: function (c) {
            ...
            if (!1 !== this.enabled) {
                ...
                if (this.enableGLHooks) {
                    ...
                    f.drawArrays = function (mode, first, count) {
                        if (w && v && 0 === h.recordMode) {
                            if (l) {
                                var e = f.createQuery();
                                e && (h.wait && h.pending++, (v.userData.query = e), f.beginQuery(l.TIME_ELAPSED_EXT, e), J.call(this, mode, first, count), f.endQuery(l.TIME_ELAPSED_EXT));
                            } else {
                                f.finish(),
                                f.readPixels(0, 0, 1, 1, f.RGBA, f.UNSIGNED_BYTE, A),
                                (z = m.now()),
                                J.call(this, mode, first, count),
                                f.finish(),
                                f.readPixels(0, 0, 1, 1, f.RGBA, f.UNSIGNED_BYTE, A),
                                v.userData.deltaTime = (m.now() - z) / 1e6,
                                h.needsUpdate = !0;
                ...
                B.setAttribute("style", "cursor: pointer; position: fixed; z-index: 99999; left: 1em; bottom: 1em; background: silver; border: 1px solid gray;");
        ...
        begin: function () {
            this.mounted &&
                ...
                0 < c.x && 0 < c.y && ((this.needsRecord = !1), (this.frameIndex = 0), this.recordCount++, c.set(c.x * this.scale, c.y * this.scale).floor(), c.x !== this.resolution.x || c.y !== this.resolution.y)) &&
        ...
        update: function () {
            if (!1 !== this.enabled && (this.updatePreview || this.autoUpdatePreview)) {
                ...
                60 < this.updateFrameIndex && ((this.maxDeltaLabel = "Max. ms: " + (this.maxDelta / 1e6).toFixed(6)), (this.updateFrameIndex = 0));
    ...
    THREE_GLOBAL.DrawCallInspector = d;
    // expose metrics via getters
    Object.defineProperty(d.prototype, 'drawCallCount', {
        get() { return this.calls; },
    });
    Object.defineProperty(d.prototype, 'maxDeltaTime', {
        // in seconds: max GPU time seen (ms → s)
        get() { return this.maxDelta / 1e6; }
    });
})();

export default THREE.DrawCallInspector;
