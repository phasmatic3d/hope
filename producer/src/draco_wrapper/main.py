            ...
            while True:
                ...
                if encoding_mode is EncodingMode.FULL:
                    ...
                    if(points_full_frame.any()):
                        buf_full = draco_full_encoding.encode(points_full_frame, colors_full_frame)
                        # Broadcast
                        server.broadcast(bytes([1]) + buf_full) # prefix with single byte to understand that we are sending one buffer
