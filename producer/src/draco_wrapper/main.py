...
def full_frame_compression(
    points: np.ndarray, 
    colors: np.ndarray,
    encoder_wrapper: DracoWrapper,
) -> bytes:
    ...
    buffer = encoder_wrapper.encode(
        points=points, 
        colors=colors
    )

    #TODO I don't think these are needed
    #draco_full_encoding.compression_stats.compression_ms = (time.perf_counter() - compression_time_start) * 1000 # encoding end
    #draco_full_encoding.compression_stats.encoded_bytes = len(buf_all)
    #pipeline_stats.compression_ms = (time.perf_counter() - compression_time_start) * 1000 
...
def encode_point_cloud(
    ...
    #TODO if you change to process pool the data for the stats 
    #gets discarded because there is no helper like _encode_chunk
    ...
    with Live(refresh_per_second=1, screen=False) as live:
        try:
            while True:
                ...
                # Clip to valid range
                column_coordinates_indices = np.clip(column_coordinates, 0, depth_width-1)
                row_coordinates_indices = np.clip(row_coordinates, 0, depth_height-1) # 1ms

                pixel_indices = row_coordinates_indices * depth_width + column_coordinates_indices
                ...
                flat_color_pixels = color_bgr.reshape(-1, 3)

                # Select the BGR color for each point by its flat pixel index, then convert to RGB
                colors = flat_color_pixels[pixel_indices][:, ::-1]  # shape: (N, 3) in [R, G, B] order
                ...
                valid = (
                    ...
                    & (numpy_vertices[:, 2] > 0)             # Z > 0 (point lies in front)
                )
                #valid = (
                ...
                if encoding_mode is EncodingMode.FULL:
                    ...
                    build_mask_for_roi_start_time = time.perf_counter()
                    ...
                    #TODO simplification without the divmod check it out
                    in_roi = (
                        valid
                        & (column_coordinates_indices >= x0) & (column_coordinates_indices <  x1)
                        & (row_coordinates_indices    >= y0) & (row_coordinates_indices    <  y1)
                    )
                    ...
                    colors_out_roi = colors        [out_roi]  # ~10 ms

                    pipeline_stats.build_mask_for_roi_ms = (time.perf_counter() - build_mask_for_roi_start_time) * 1000
