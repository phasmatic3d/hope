...
class SAM2CameraPredictor(SAM2Base):
    ...
    def prepare_data(self, img, image_size, img_mean=(0.485, 0.456, 0.406), img_std=(0.229, 0.224, 0.225)):
        # 1. Handle Torch Tensor
        if isinstance(img, torch.Tensor):
            # Ensure it is float
            img = img.float()
            
            # Capture original dimensions (Assuming C, H, W)
            height, width = img.shape[-2:]
            
            # Resize: Interpolate requires (Batch, Channel, Height, Width)
            if img.ndim == 3:
                img = img.unsqueeze(0)
            ...
            # Normalize to 0-1 if it isn't already
            if img.max() > 1.0:
                img /= 255.0
                
        # 2. Handle Numpy/PIL (Existing Logic)
        else:
        ...
        # 3. Standardization (Device Aware)
        # Important: Create mean/std on the same device/dtype as the image
        img_mean = torch.tensor(img_mean, device=img.device, dtype=img.dtype)[:, None, None]
        img_std = torch.tensor(img_std, device=img.device, dtype=img.dtype)[:, None, None]
